...
const Validate_plan_delete = require("../../../middleware/joi_validation/Admin/Plan/Joi_validate_delete_plan");
  ...
  try {
      ...
      return next(
        new ApiErrors(
          JSON.stringify({
            ...
            arabic: "... عذرا لم يتم العثور على حساب الاددمن",
          }),
          404
        )
      );
    ...
    if (plan.teachers.length > 0) {
      for (let i = 0; i < plan.teachers.length; i++) {
        ...
        if (teacher) {
          teacher.my_plans = teacher.my_plans.filter(
            (plan_id) => plan_id != plan._id
          );
        }
    ...
    if (plan.students.length > 0) {
      for (let i = 0; i < plan.students.length; i++) {
        ...
        // check if the
        if (student) {
          student.plans = student.plans.filter(
            (plan_id) => plan_id != plan._id
          );
        }
