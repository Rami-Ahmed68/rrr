...
const Validate_parent_delete = require("../../../../middleware/joi_validation/Admin/Parent/Joi_validate_delete_parent");
...
router.delete("/", async (req, res, next) => {
  try {
    ...
    // check if the  admin id in token is eqaul idin body
    ...
    if (!isAdmin) {
      ...
      return next(
        new ApiErrors(
          JSON.stringify({
            english:
              "Sorry, you don't have permissions to delete parent account ...",
            arabic: "... عذرا ليس لديك الصلاحيات لحذف حساب ولي الأمر",
          }),
          403
        )
      );
    ...
    if (
      ...
      parent.avatar != process.env.DEFAULT_WOMAN_AVATAR
    ) {
